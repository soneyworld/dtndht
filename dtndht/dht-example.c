...
#define PRINTHASH(a) printf("0X%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x",(uint8_t)a[0],(uint8_t)a[1],(uint8_t)a[2],(uint8_t)a[3],(uint8_t)a[4],(uint8_t)a[5],(uint8_t)a[6],(uint8_t)a[7],(uint8_t)a[8],(uint8_t)a[9],(uint8_t)a[10],(uint8_t)a[11],(uint8_t)a[12],(uint8_t)a[13],(uint8_t)a[14],(uint8_t)a[15],(uint8_t)a[16],(uint8_t)a[17],(uint8_t)a[18],(uint8_t)a[19]);
...
static volatile sig_atomic_t dumping = 0, searching = 0, exiting = 0, announcing=0;
...
const unsigned char hash[20] = {
    0x51, 0x57, 0x87, 0x89, 0xdf, 0xc4, 0x2a, 0xee, 0xf6, 0x03,
    0x1f, 0x81, 0x94, 0xa9, 0x3a, 0x16, 0x1, 0x8b, 0x72, 0x7b
};
         ...
         void *data, size_t data_len)
{
    ...
    if(event == DHT_EVENT_SEARCH_DONE) {
        printf("Search done for infohash "); PRINTHASH(info_hash); puts("");
    }
    else if(event == DHT_EVENT_VALUES) {
    	printf("Values for infoshash "); PRINTHASH(info_hash); puts("");
        printf("   -> %d values.\n", (int)(data_len / 6));
        for (i=0; i< data_len/6; i++) { // /6 should be ok, this event is pnly for ipV4
        printf("   -> %i:  %i.%i.%i.%i Port %i\n", i,*(currPtr+0),*(currPtr+1),*(currPtr+2),*(currPtr+3), ntohs(*(uint16_t *)(currPtr+4)));
        currPtr+=6;
        }
   }
}
...
main(int argc, char **argv)
{
    ...
    while(1) {
        ...
        if(searching) {
        	printf("*** SEARCHING FOR "); PRINTHASH(hash); puts("");
            if(s >= 0)
        ...
        if (announcing) {
        	printf("*** WILL ANNOUNCE port 5555 for ");PRINTHASH(hash); puts("");
        	if(s >= 0)
                dht_search(hash, 5555, AF_INET, callback, NULL);
            if(s6 >= 0)
                dht_search(hash, 5555, AF_INET6, callback, NULL);
            announcing = 0;
        }
